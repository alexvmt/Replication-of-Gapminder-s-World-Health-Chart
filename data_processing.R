### Data Processing for the Replication of the Gapminder World Health Chart
...
region.income_level <- region.income_level.data.raw %>%
  ...
  select(country_name, region, income_level) 
...
data.final <- na.omit(data.cleaned.joined)

# remove objects that are not needed anymore
rm(region.income_level.data.raw, population.data.raw, life_expectancy.data.raw, income.data.raw, region.income_level, population, life_expectancy, income, j1, j2, data.cleaned.joined)
...
### testwise create world health chart for a certain year with ggplot2

# set year
Year <- 2018

# set up png file
png(file=paste0("world_health_chart_",Year,".png"), width=700, height=500)

# select data for certain year
world_health_chart_data <- data.final %>% 
  filter(year==Year)
...
write_csv(data.final, "gapminder_world_health_chart_data_processed.csv")